    ...
    html:
      message
        ? (gameIndex // if gameindex is sent, that means a user was invited to resume an exiting game
          ? `<h1>Hexology: The Hex Game That Runs In Your Browser</h1><br>${message}<p><a href="http://localhost:3000/game/room?${room}/=${gameIndex}=${otherUser}">Join the game!</a></p>`
          : `<h1>Hexology: The Hex Game That Runs In Your Browser</h1><br>${message}<p><a href="http://localhost:3000/game/room?${room}">Join the game!</p>`)
        : template
  }
